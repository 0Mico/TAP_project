...
class DatasetPreparator:
    ...
    def create_datasets(self, test_size: float = 0.15, random_seed: int = 1) -> Tuple[SkillNERDataset, SkillNERDataset]:
        """
...
class SkillExtractorTrainer:
    ...
    def extract_skills(self, text: str, model_path: str = None) -> List[str]:
        ...
        for token, label in zip(tokens, predicted_labels):
            ...
            if label == "B-SKILL":
                if current_skill:
                    skill_text = ''.join(current_skill).replace('##', '')
...
def main():
    ...
    train_dataset, val_dataset = preparator.create_datasets(test_size=0.2)
    ...
    '''for i, text in enumerate(test_examples, 1):
        ...
        skills = trainer.extract_skills(text, model_path="./skill_extractor_model")
